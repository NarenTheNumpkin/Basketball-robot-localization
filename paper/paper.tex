...
\toprule
\textbf{Author \& Year} & \textbf{Camera Type} & \textbf{Methodology} & \textbf{Performance Highlights} & \textbf{Limitations} \\ 
\midrule
\addlinespace
...
It uses a ResNet-based encoder-decoder architecture, where:
\begin{itemize}
...
\section*{Appendix}
\subsection*{A. t-test for Correlation Significance}
\par \noindent
...
\subsubsection*{1. Iterations vs. x loss}
\begin{itemize}
    \item \textbf{Number of data pairs (n):} 9
    \item \textbf{Pearson's r:} -0.32
    \item \textbf{Degrees of Freedom (df):} $9-2=7$
\end{itemize}
