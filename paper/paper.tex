...
Figure~\ref{fig:x_loss} and Figure~\ref{fig:y_loss} show the%  graphical repr% esentation of this loss.
\par \noindent
...
$$ t = -0.67 \frac{\sqrt{9-2}}{\sqrt{1 - (-0.67)^2}} = \frac{-1.7727}{0.7424} \approx -2.39 $$

\subsection{Comparision and Analysis}
\par \noindent
Two models were chosen as a baseline, EfficientNet-B0 and MobileNetV2 due to their ability to run on 
efficiently edge devices which is particularly suitable for faster inference time.
\subsubsection{MobilenetV2}
\par \noindent
MobilenetV2 is built on depthwise separable convolutions and inverted residual blocks with linear bottlenecks. A standard $k{\times}k$ convolution with $M$ input and $N$ output channels over a $D_F{\times}D_F$ map costs
\begin{equation}
\mathrm{Cost}_{\text{std}} = D_K^2 \cdot M \cdot N \cdot D_F^2
\end{equation}
Depthwise separable convolutions reduce this to
\begin{equation}
\mathrm{Cost}_{\text{dw-sep}} = D_K^2 \cdot M \cdot D_F^2 \;+\; M \cdot N \cdot D_F^2
\end{equation}
% With width multiplier $\alpha\!\in\!(0,1]$ and resolution multiplier $\rho\!\in\!(0,1]$ (as in MobilenetV2), the cost is:
% \begin{equation}
% \mathrm{Cost}_{\alpha,\rho} = D_K^2 \cdot (\alpha M) \cdot (\rho D_F)^2 \;+\; (\alpha M)\cdot(\alpha N)\cdot(\rho D_F)^2.
% \end{equation}
A depthwise separable block can be written as
\begin{equation}
y = \mathrm{Conv}_{1\times1}\!\Big(\sigma\!\big(\mathrm{BN}(\mathrm{Conv}_{\text{dw}}(x))\big)\Big)
\end{equation}
where BN is batch normalization and $\phi$ is a non linear operator
MobilenetV2’s inverted residual with linear bottleneck has the operator flow
\begin{align}
z &= \phi\!\big(\mathrm{BN}(\mathrm{Conv}^{\text{expand}}_{1\times1}(x))\big) \\
u &= \phi\!\big(\mathrm{BN}(\mathrm{Conv}^{\text{dw}}_{k\times k}(z))\big) \\
y &= \mathrm{BN}(\mathrm{Conv}^{\text{proj}}_{1\times1}(u))
\end{align}

\subsubsection{EfficientNet}
\par \noindent
EfficientNet scales a baseline using compound scaling of depth ($d$), width ($w$), and resolution ($r$). For a global coefficient $\phi \ge 0$ and constants $\alpha,\beta,\gamma>0$:
\begin{align}
d &= \alpha^{\phi}, \\
w &= \beta^{\phi}, \\
r &= \gamma^{\phi}, \\
&\text{subject to } \alpha \cdot \beta^{2} \cdot \gamma^{2} \approx \kappa.
\end{align}
The FLOPs then scale approximately as:
\begin{equation}
\mathrm{FLOPs}(\phi) \propto d \cdot w^{2} \cdot r^{2}
= \alpha^{\phi}\beta^{2\phi}\gamma^{2\phi}.
\end{equation}
The backbone uses MBConv (inverted residual) blocks with squeeze-and-excitation (SE) and SiLU/Swish. A simplified MBConv with SE:
\begin{align}
z &= \phi\!\big(\mathrm{BN}(\mathrm{Conv}^{\text{expand}}_{1\times1}(x))\big), \\
u &= \phi\!\big(\mathrm{BN}(\mathrm{Conv}^{\text{dw}}_{k\times k, s}(z))\big), \\
s &= \sigma\!\big(W_2\,\delta(W_1\,\mathrm{GAP}(u))\big) \odot u, \\
y &= \mathrm{BN}(\mathrm{Conv}^{\text{proj}}_{1\times1}(s)).
\end{align}
Skip connection:
\begin{equation}
\hat y =
\begin{cases}
x + y, & \text{if } s=1 \text{ and } C_{\text{out}}=C_{\text{in}},\\
y, & \text{otherwise.}
\end{cases}
\end{equation}
