...
\par \noindent
Figure VIII depicts the loss at each epoch throughout the training process.
8 independent images at different points of the court were again captured 
from the simulation and fed into the model, Figure IX shows the plot between
the ground truth and the prediction made by the model for 8 images in an iteration.

\par \noindent
Table 1 illustrates the x and y losses for the corresponding number of iterations of images fed
into the model, Figure VI and VII show the graphical representation of this.
...
\begin{table}[H]
\centering
\captionof{table}{Prediction Loss}
...
{ \centering
 ...
 \captionof{figure}{x loss}\label{pinki}
}
